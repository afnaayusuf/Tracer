#!/usr/bin/env bash
# =====================================================================================
#  Tracer / vi-engine — session 06b build (replaces build_session_06.sh, which stopped at the
#  `frame` run): heartbeat scheduler, hybrid ROI+heartbeat detection, open grace window,
#  continuity metrics, fake detector + smoke tests, AND the fix for the crash: every call to the
#  traced RF-DETR is padded to its batch size, so a single full-frame heartbeat image works.
#  Safe to run on a runtime where session 06 was half-applied: it commits that partial state
#  first if all dirty files are session-06 files, then overwrites them with the fixed versions.
#
#  Colab: paste into one %%bash cell after the env cell, or: bash /content/build_session_06b.sh
...
step "1. audit (with recovery from an interrupted session 06 run)"
ALLOW_DIRTY='
...
SPEC.md
EDGE_CASES.md
colab/sessions/session_06_build.sh
'
if [ -n "$(git status --porcelain)" ]; then
  bad=0
  while read -r st path; do
    [ -z "$path" ] && continue
    case "$path" in schemas/*|data/*) continue;; esac
    echo "$ALLOW_DIRTY" | grep -x "$path" >/dev/null || { warn "unexpected dirty file: $path"; bad=1; }
  done <<< "$(git status --porcelain | sed 's/^...//')"
  if [ "$bad" = 1 ] && [ "${FORCE:-0}" != "1" ]; then die "dirty files outside session 06; inspect or rerun with FORCE=1"; fi
  git add -A
  git commit -qm "$SESSION (partial, from interrupted run)" && echo "committed the interrupted session-06 state"
fi
...
EOF_VI
cat > vi/detect/rfdetr.py << 'EOF_VI'
from __future__ import annotations
...
EOF_VI
cp "$0" colab/sessions/session_06b_build.sh 2>/dev/null || true
...
if git diff --cached --quiet; then echo "nothing to commit"; else
  git commit -qm "$SESSION: heartbeat scheduler (R10), hybrid ROI+full-frame detection, open grace window, continuity metrics, traced-batch padding fix, fake detector + slice smoke tests"
fi
