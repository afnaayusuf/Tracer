#!/usr/bin/env bash
# =====================================================================================
#  Tracer / vi-engine — session 19 build:
#   * vLLM venv: torch matched to the DRIVER again (vLLM 0.30's wheel links libcudart.so.13; forcing
#     cu128 broke vllm._C); a venv where `import vllm` fails is rebuilt; clean launch env kept
#   * agent: answers up to 4000 chars; an over-long answer is truncated and accepted, not retried
#     seven times; the replay prints validation reasons
#   * scenario rules: numbers must stand alone (00:14.0 is not "14")
#   * bench/cast_sheet.py: one image of every person tube's birth crop with id/life/quality -> upload it
...
#    !bash /content/build_session_19.sh
# =====================================================================================
set -euo pipefail
...
[ -n "$(git log --grep='^session 18' --format=%h)" ] || die "session 18 commit not found; run build_session_18.sh first"
EXPECTED='
.gitignore
Makefile
PLAN.md
...
if __name__ == "__main__":
    main()
EOF_VI
cat > bench/cast_sheet.py << 'EOF_VI'
...
cp "$0" colab/sessions/session_19_build.sh 2>/dev/null || true
...
if [ "$HAS_GPU" = 1 ] && [ -f "$SOURCE" ]; then
  ...
  python bench/cast_sheet.py data/episodes/*.jsonl --keyframes data/keyframes --out data/bench/cast_sheet.jpg
  echo "  -> download /content/Tracer/data/bench/cast_sheet.jpg from the Files panel and upload it to the chat"
else
  warn "GPU or clip missing (step 0); using the CPU slice episode"
  rm -rf data/episodes; python bench/slice_cpu.py >/dev/null
fi

step "6. reasoning backend: vLLM (driver-matched torch, clean env) -> transformers -> fake"
BACKEND="${BACKEND:-}"
if [ -z "$BACKEND" ]; then
...
if git diff --cached --quiet; then echo "nothing to commit"; else
  git commit -qm "$SESSION: driver-matched vLLM torch (rebuild on import failure), answer salvage, stand-alone number rules, cast sheet"
fi
if [ "${NO_PUSH:-0}" != "1" ] && [ -n "${GH_TOKEN:-}" ]; then git push -q && echo "pushed" || warn "push failed"; else warn "not pushed (NO_PUSH or no GH_TOKEN)"; fi

step "10. summary"
echo "HEAD: $(git log -1 --oneline)"
echo "paste steps 5, 6, 7, 8 back and upload data/bench/cast_sheet.jpg."
