#!/usr/bin/env bash
# =====================================================================================
#  Tracer / vi-engine — session 17 build (superset of 16 + 16b; needs only session 15 on GitHub):
#   * vLLM venv: torch build chosen from the driver's CUDA (uv --torch-backend=auto); a venv whose
#     torch is newer than the driver is rebuilt; failures print the root-cause lines
#   * Qwen3.5 thinking mode OFF on both backends (it ate the whole token budget before any JSON);
#     think blocks stripped; unknown tool args dropped with a note instead of failing the call
#   * quality flags, ground truth (8 people / 6 whole time / 0 pickups), latency, scenario eval
#
#  ONE-CELL FORM (Python cell, so the token cannot be forgotten):
...
#    !bash /content/build_session_17.sh
#  Env: GH_REPO GH_TOKEN REPO_DIR SOURCE DB_URL AGENT_MODEL SKIP_VLLM=1 BACKEND NO_PUSH=1 FORCE=1
# =====================================================================================
set -euo pipefail
...
[ -n "${GH_TOKEN:-}" ] || warn "GH_TOKEN is not set: the session will commit locally but cannot push (use the one-cell form above)"
...
questions:
  ...
  - q: Who was at the conveyor on the right, and when did they arrive?
    must_mention: [["00:0", "start", "beginning", "0.0", "from the first"]]   # the hard-hat worker is there from 0 s
    must_not_mention: []
    must_cite_prefix: ["cam1:E"]
...
cp "$0" colab/sessions/session_17_build.sh 2>/dev/null || true
...
step "7. reasoning backend: vLLM (driver-matched torch) -> transformers (thinking off) -> fake"
BACKEND="${BACKEND:-}"
if [ -z "$BACKEND" ]; then
...
step "9. scenario acceptance against ground truth (8 people, 6 whole time, 0 pickups, 10 s budget)"
...
if git diff --cached --quiet; then echo "nothing to commit"; else
  git commit -qm "$SESSION: sessions 16/16b + driver-matched vLLM torch, no-thinking templates, think stripping, unknown args dropped, ground truth"
fi
if [ "${NO_PUSH:-0}" != "1" ] && [ -n "${GH_TOKEN:-}" ]; then git push -q && echo "pushed" || warn "push failed"; else warn "not pushed (NO_PUSH or no GH_TOKEN)"; fi

step "11. summary"
echo "HEAD: $(git log -1 --oneline)"
echo "paste steps 0, 6, 7, 8 and 9 back."
